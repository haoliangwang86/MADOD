...
class Job:
    ...
    @staticmethod
    def launch_same_type_gpu_jobs(jobs, launcher_fn, first_gpu_id, n_gpus, jobs_per_gpu):
        print('Launching...')
        jobs = jobs.copy()
        np.random.shuffle(jobs)
        print('Making job directories:')
        for job in tqdm.tqdm(jobs, leave=False):
            os.makedirs(job.output_dir, exist_ok=True)
        commands = [job.command_str for job in jobs]
        launcher_fn(commands, first_gpu_id, n_gpus, jobs_per_gpu)
        print(f'Launched {len(jobs)} jobs!')
...
def make_args_list(n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
    data_dir, task, holdout_fraction, single_test_envs, hparams, save_model_every_checkpoint, ood_classes):
    args_list = []
                ...
                for test_envs in all_test_envs:
                    for hparams_seed in range(n_hparams_from, n_hparams):
                        ...
                        if ood_classes is not None:
                            train_args['ood_classes'] = ood_classes
...
if __name__ == "__main__":
    ...
    parser.add_argument('--auto_rerun_incomplete', action='store_true')
    parser.add_argument('--first_gpu_id', type=int, default=0)
    ...
    if args.command == 'launch':
        ...
        if args.command_launcher == "same_type_gpu":
            Job.launch_same_type_gpu_jobs(to_launch, launcher_fn, args.first_gpu_id, args.n_gpus, args.jobs_per_gpu)
        else:
            Job.launch(to_launch, launcher_fn)

        # automatically rerun incomplete jobs
        if args.auto_rerun_incomplete:
            while True:
                jobs = [Job(train_args, args.output_dir) for train_args in args_list]
                print("Relaunching failed jobs.")
                to_delete = [j for j in jobs if j.state == Job.INCOMPLETE]
                if not len(to_delete):
                    print("No failed jobs. All jobs finished.")
                    break
