...
class LeaveOneOutSelectionMethod(SelectionMethod):
    ...
    def _step_acc(self, records):
        ...
        to a single step."""
        test_records = get_test_records(records)  # records with only one test env
        if len(test_records) != 1:
            return None

        test_env = test_records[0]['args']['test_envs'][0]
        n_envs = 0
        for i in itertools.count():  # count the number of envs (including test env)
            if f'env{i}_out_acc' not in records[0]:
                break
            n_envs += 1
        val_accs = np.zeros(n_envs) - 1  # all negative 1
        for r in records.filter(lambda r: len(
                r['args']['test_envs']) == 2):  # filter records with two test envs, one must be the "test_env"
            val_env = (set(r['args']['test_envs']) - set([test_env])).pop()  # the other test_env will be val_env
            val_accs[val_env] = r['env{}_in_acc'.format(
                val_env)]  # update the val_acc of using val_env as validation environment using val_env's in_acc
        val_accs = list(val_accs[:test_env]) + list(val_accs[test_env + 1:])  # exclude the -1 value for the test_env
        if any([v == -1 for v in val_accs]):
            return None
        val_acc = np.sum(val_accs) / (n_envs - 1)  # average val_acc
        return {
            'val_acc': val_acc,
            'test_acc': test_records[0]['env{}_in_acc'.format(test_env)]  # test_acc is the test_env in_acc
        }
