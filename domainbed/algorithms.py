...
class MBDG_Base(ERM):
    ...
    # for MNIST
    def plot_mixup_oods(self, x, x_neg, gmm, thr, num_classes):
        delta = torch.randn(x.size(0), self.G.delta_dim, 1, 1).to(x.device).requires_grad_(False)
        images_to_plot = [x, x_neg]
        images_to_plot.append(self.G.mnist_mixup_ood(x, x_neg, delta, gmm, thr, num_classes))
    ...
    #     delta = torch.randn(x.size(0), self.G.delta_dim, 1, 1).to(x.device).requires_grad_(False)
    #     images_to_plot = [x, x_neg]
    #     outputs, random_number_1, random_number_2 = self.G.mnist_mixup_ood(x, x_neg, delta, gmm, thr, num_classes)
    #     images_to_plot.append(outputs)
    #     title_list = ['x1', 'x2', f'OOD: {int(random_number_1)} x1 + {int(random_number_2)} x2']
    #
    #     def plot_mixup_images(image_list, title_list, save_dir="domainbed/mixup_imgs"):
    #         os.makedirs(save_dir, exist_ok=True)
    #         # Save each image in the batch
...
class MADOD(MBDG_Base):

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(MADOD, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.dual_var = torch.tensor(0.001).cuda().requires_grad_(False)
        self.dual_var_2 = torch.tensor(0.01).cuda().requires_grad_(False)

    def ignore_last_logit(self, logits):
        return logits[:, :-1]

    def dist_reg_no_last_way(self, x, clean_output):
        mb_images = self.generate_images(x)
        mb_logits = self.ignore_last_logit(self.predict(mb_images))
        mb_output = F.softmax(mb_logits, dim=1)
        return self.kl_div(F.softmax(clean_output, dim=1), mb_output)
    ...
    def update(self, tasks):
        ...
        for task in tasks:
            x_spt, y_spt, x_qry, y_qry, x_psd = task

            # Create a copy of the classifier for this task
            fast_classifier = copy.deepcopy(self.classifier)
            fast_optimizer = torch.optim.SGD(fast_classifier.parameters(), lr=self.hparams["inner_lr"])

            self.featurizer.eval()  # freeze featurizer parameters
            fast_classifier.train()

            # Inner loop: update fast_classifier
            for _ in range(self.hparams["n_inner_updates"]):
                # Don't calculate gradients to save computation
                with torch.no_grad():
                    features_spt = self.featurizer(x_spt)

                logits = fast_classifier(features_spt)
                ce = F.cross_entropy(logits, y_spt)
                dist_reg = self.calc_dist_reg(x_spt, logits)
                loss = ce + self.dual_var * dist_reg
                fast_optimizer.zero_grad()
                loss.backward()
                # torch.nn.utils.clip_grad_norm_(fast_classifier.parameters(), max_norm=self.hparams["max_norm"])
                fast_optimizer.step()

            self.featurizer.train()  # unfreeze featurizer parameters
            features_qry = self.featurizer(x_qry)
            features_psd = self.featurizer(x_psd)

            # Evaluate updated model on query set
            logits_q = fast_classifier(features_qry)
            ind_scores = -torch.logsumexp(logits_q, dim=1)

            logits_p = fast_classifier(features_psd)
            ood_scores = -torch.logsumexp(logits_p, dim=1)

            ce_q = F.cross_entropy(logits_q, y_qry)
            ...
            ood_reg = torch.pow(F.relu(ind_scores - self.hparams['m_in']), 2).mean() + \
                      torch.pow(F.relu(self.hparams['m_out'] - ood_scores), 2).mean()
            loss_q = ce_q + self.dual_var_2 * ood_reg
            losses_q.append(loss_q)

            # Calculate accuracy
            pred_q = logits_q.argmax(dim=1)
            accuracy_q = (pred_q == y_qry).float().mean()
            accuracies_q.append(accuracy_q)

        # Outer loop: update original classifier
        mean_loss_q = torch.stack(losses_q).mean()
        mean_accuracy_q = torch.stack(accuracies_q).mean()
        self.optimizer.zero_grad()
        mean_loss_q.backward()
        # torch.nn.utils.clip_grad_norm_(self.network.parameters(), max_norm=self.hparams["max_norm"])
        self.optimizer.step()

        return {'train_q_loss': mean_loss_q.item(),
                'train_q_acc': mean_accuracy_q.item()}

    def finetune(self, task):
        x_spt, y_spt, x_qry, y_qry, x_psd = task

        # Update the classifier directly instead of creating a copy
        fast_optimizer = torch.optim.SGD(self.classifier.parameters(), lr=self.hparams["inner_lr"])
        ...
        for _ in range(self.hparams["n_inner_updates"]):
            ...
            logits = self.classifier(features_spt)
            ce = F.cross_entropy(logits, y_spt)
            dist_reg = self.calc_dist_reg(x_spt, logits)  # original version
            loss = ce + self.dual_var * dist_reg
            fast_optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.classifier.parameters(), max_norm=self.hparams["max_norm"])
            fast_optimizer.step()

        self.classifier.eval()
        with torch.no_grad():
            features_qry = self.featurizer(x_qry)
            logits_q = self.classifier(features_qry)
            ce_q = F.cross_entropy(logits_q, y_qry)
            dist_reg_q = self.calc_dist_reg(x_qry, logits_q)  # original version
            loss_q = ce_q + self.dual_var * dist_reg_q

            # Calculate accuracy
            pred_q = logits_q.argmax(dim=1)
            accuracy_q = (pred_q == y_qry).float().mean()

        return {'test_q_loss': loss_q.item(),
                'test_q_acc': accuracy_q.item()}
    ...
    #
    #             logits = fast_classifier(features_spt)
    #             # logits = self.ignore_last_logit(logits)
    #             ce = F.cross_entropy(logits, y_spt)
    #             loss = ce
    #             fast_optimizer.zero_grad()
    #             loss.backward()
    ...
    #         # Evaluate updated model on query set
    #         logits_q = fast_classifier(features_qry)
    #         # logits_q = self.ignore_last_logit(logits_q)
    #
    #         ce_q = F.cross_entropy(logits_q, y_qry)
    #
    #         loss_q = ce_q
